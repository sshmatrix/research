...
\begin{flushleft}
	...
	2. for any two points a and b on the elliptic curve, a line through them intersects the elliptic curve again at most - but not necessarily - 1 more point, denoted by c; the blue line in figure \ref{fig:ecc0} is one such example.
	}}
\end{flushleft}
\begin{multicols}{2}
	\noindent
	\begin{minipage}{\linewidth}
		...
		\captionof{figure}{\mono{Elliptic curve over a finite prime field modulo {\pbold{p}}; the effect of the modulo operator \% is to "wrap" the out-of-range values to within range. In the above example, it takes 5 'wrappings' and 1 final reflection to calculate -c.}}
		\label{fig:ecc1}
	\end{minipage}
	\linebreak\linebreak
	...
	\begin{flushright}
		{\vspace{-8mm}\mono{(2)}}
	\end{flushright}
	\textls[-50]{\mono{This is the same as saying that $\oplus$ operation between a and b is equivalent to taking reflection (-) of the third intersection point c across x-axis. Most literature phrases this $\oplus$ operation as the new "addition" operation on the group elements of y. However, we noted before that the third intersection c doesn't exist for all pairs a and b; in jargon-speak, we say that 'the group is not closed under the $\oplus$ operation'.
	}}
\end{multicols}
\begin{flushleft}
	\textls[-50]{\mono{We can solve this closeness problem by explicitely including the infinity element $\infty$ to the group. Once we have access to $\infty$, all intersections similar to the green line are assumed to intersect the curve again at c = $\infty$, such that
	}}
	\begin{center}
...
\end{flushleft}
\begin{flushleft}
	\textls[-50]{\mono{Note that $\infty$ acts like the identity element \^I of our new group since it satisfies a $\oplus$ \^I = a. Lastly, the third case of the red line is not difficult since it is simply a special case of the blue line when b = c; another important tangent case arises when a = b, such that a $\oplus$ a = c. Now we have all the ingredients required to construct a robust inverse problem on our new group y with the permitted operation $\oplus$. We reinstate the requirement of evaluating the elliptic curve on a finite prime field by explicitely defining a finite field range and wrapping the out-of-range evaluations over the elliptic curve with the modulo operator \% {\pbold{p}}. The final form of the $\oplus$ operation is shown in figure \ref{fig:ecc1}. In a real world ECC implementation, this process is carried out a large number of times (assuming a = b; take tanget at point a) making the forward process increasingly complex and the resulting inverse problem increasingly difficult to compute.\linebreak\linebreak
	}}
	...
	\textls[-50]{\mono{The 'large number of repetitions' is assigned as the secret private key K\textsubscript{sec} and the public key K\textsubscript{pub} is equivalent to calculating (a $\oplus$ a) at a public value a = a\textsubscript{o} repeated N number of times\textsuperscript{\textcolor{blue}{4}}\blfootnote{\textls[-50]{\mono{\textsuperscript{4}a = b assertion is made purely for simplicity since having a $\neq$ b doesn't provide extra security}}}, i.e. K\textsubscript{pub} = (...((a\textsubscript{o} $\oplus$ a\textsubscript{o})\textsubscript{1} $\oplus$ a\textsubscript{o})\textsubscript{2} $\oplus$ a\textsubscript{o})\textsubscript{3} ...)\textsubscript{N}. The exact inverse problem - called the elliptic curve discrete logarithm problem - is equivalent to finding the number of repetitions it took to arrive at K\textsubscript{pub} from a publicly known value a\textsubscript{o}. Is this inverse problem hard? It appears that given knowledge of K\textsubscript{pub} and a\textsubscript{o}, one can easily revert the process described in figure \ref{fig:ecc1} and derive key K\textsubscript{sec}. This is indeed true and the inverse problem only becomes hard when the coefficients in equation (1) are restricted to the prime field F\textsubscript{\pbold{p}}; more on this in the next subsection. Some of the popular elliptic curves today are specifically \code{ed25519} and \code{secp256k1}, with the later being more recent and more robust than the former.\linebreak\linebreak
	}}
	\colorbox{Gray}{\hspace{1mm}\parbox{1.0\textwidth}{\vspace{1mm}\textls[-50]{\mono{The name of the inverse problem for ECC, i.e. elliptic curve discrete logarithm problem, is highly misleading and results from purely notational reasons. The term 'logarithm' originates from interpreting the result of N times repetition of (a $\oplus$ a) operation as K\textsubscript{pub} = a\textsuperscript{N}.\vspace{1mm}}}}}\linebreak\linebreak
	 \textls[-50]{\mono{It now appears that we have achieved what we set out to do: we have found a new group who elements are now values y evaluated over the finite prime field F\textsubscript{\pbold{p}} and the new group operation (...((a\textsubscript{o} $\oplus$ a\textsubscript{o})\textsubscript{1} $\oplus$ a\textsubscript{o})\textsubscript{2} $\oplus$ a\textsubscript{o})\textsubscript{3} ...)\textsubscript{N} ($\equiv$ a\textsubscript{o}\textsuperscript{N}) is very difficult to invert. We note that the result of $\oplus$ operation on the group elements results in a value which is also a member of the group\textsuperscript{\textcolor{blue}{5}}\blfootnote{\textls[-50]{\mono{\textsuperscript{5}group is referred to as 'closed under the operation $\oplus$'}}}; this was not the case in RSA scheme where the result {\pbold{a}}$\,\times\,${\pbold{b}} is never a member of the group.\linebreak\linebreak
	}}
	\textbf{\large\pbold{ROBUSTNESS OF ECC}}\linebreak\linebreak
	\textls[-50]{\mono{The natural question to ask here is: how robust is ECC when compared to RSA and Diffie-Hellman? We remember from previous section that the inverse problem in ECC only becomes hard when the coefficients in equation (1) are also restricted to a large prime field F\textsubscript{\pbold{p}} similar to the coordinate x. To understand why this happens, simply note that for large prime values of p, q and x in equation (1), all terms on the right side are products of primes and then their sums. Similar to RSA, inverting this function numerically is equivalent to prime factorising many {\pbold{a}}$\,\times\,${\pbold{b}} products (and 'inverting' their sums). The fact that this process will have to be repeated a very large number of times (at least K\textsubscript{sec} times) to potentially brute-force for the private key, makes the ECC algorithm several orders of magnitudes more secure than RSA or Diffie-Hellman. Having said that, this several magnitudes of improvement is not sufficient to survive the post-quantum era; this finally brings us to the world of zero-knowledge cryptography.\linebreak\linebreak
	Curious observers may note that unlike RSA where we keep both {\pbold{a}} and {\pbold{b}} secret (and reveal their product {\pbold{a}}$\,\times\,${\pbold{b}}), in ECC we additionally also reveal one of the independent variables a\textsubscript{o}, i.e. a\textsubscript{o}\textsuperscript{N} and a\textsubscript{o} are both known, leaving only N to be discovered. Why is this the case? The answer lies in the fact that the solutions {\pbold{a}} and {\pbold{b}} of the product {\pbold{a}}$\,\times\,${\pbold{b}} are unique while the solutions a and N of the expression a\textsuperscript{N} are not unique unless either a or N is specified. Uniqueness of the keypairs is a very important property that cannot be violated for cryptosystems; this requires a\textsubscript{o} to be fixed and specified publicly in ECC algorithms.\linebreak\linebreak\linebreak
	}}
	\textbf{\Large\pbold{ZERO-KNOWLEDGE CRYPTOGRAPHY}}\linebreak\linebreak
	\textls[-50]{\mono{Inshallah
	}}
\end{flushleft}
\begin{flushleft}
