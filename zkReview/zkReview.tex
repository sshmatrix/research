...
\begin{flushleft}
	...
	\textls[-50]{\mono{Cryptographers realised back in the day that prime numbers were one such system that could provide the desired functionality. For instance, consider two sufficiently large prime numbers {\pbold{a}} = 53781811 and {\pbold{b}} = 23252729, and their even larger product {\pbold{a}}$\,\times\,${\pbold{b}} = 1250573876312219. The product 1250573876312219 is a relatively difficult number to prime factorise back to {\pbold{a}} and {\pbold{b}} if both {\pbold{a}} and {\pbold{b}} are unknown. However, if either one of the two prime factors ({\pbold{a}} or {\pbold{b}}) are known, then it is straightforward to calculate the other unknown prime factor by simple division. To intuitively understand this system further, let's break it down into its principle components: we took a set of very large prime numbers of which {\pbold{a}} and {\pbold{b}} are members and defined an operation of product\textsuperscript{\textcolor{blue}{1}}\blfootnote{\textls[-50]{\mono{\textsuperscript{1}The operations of addition '+' and its inverse subtraction '-' are implictly defined}}} on the members of the set; such a finite field is called a Galois field. Additionally, we note that the product operation $\times$ is difficult to invert unless one of the two numbers is known; this kind of a system is usually called a trapdoor. Trigonometric functions like sine({\pbold{a}}) in real space and e\textsuperscript{i\pbold{a}} in complex space are some other naive examples of trapdoors.\linebreak\linebreak 
	}}
	...
	\colorbox{Gray}{\hspace{1mm}\parbox{1.0\textwidth}{\vspace{1mm}\textls[-50]{\mono{More specifically, C-DHP states that given the knowledge of {\pbold{G}}\textsuperscript{\pbold{$\bar{\text{p}}$}} and {\pbold{G}}\textsuperscript{\pbold{$\bar{\text{q}}$}} (but not {\pbold{$\bar{\text{p}}$}} and {\pbold{$\bar{\text{q}}$}}), it is difficult to compute the value of {\pbold{G}}\textsuperscript{\pbold{$\bar{\text{p}}\bar{\text{q}}$}}. More simply, if we are revealed the values of two elements derived from secretly taking \pbold{$\bar{\text{p}}$}} and {\pbold{$\bar{\text{q}}$}} cyclic operations, then it is difficult to compute the element derived by taking {\pbold{$\bar{\text{p}}$}}$\,\times\,${\pbold{$\bar{\text{q}}$}} cyclic operations. One variant of Diffie-Hellman problem which is easy to solve in this case is the Decisional Diffei-Hellman Problem (D-DHP). D-DHP states that given knowledge of three elements {\pbold{G}}\textsuperscript{\pbold{$\bar{\text{p}}$}}, {\pbold{G}}\textsuperscript{\pbold{$\bar{\text{q}}$}} and {\pbold{G}}\textsuperscript{\pbold{$\bar{\text{r}}$}} (but not {\pbold{$\bar{\text{p}}$}}, {\pbold{$\bar{\text{q}}$}} or {\pbold{$\bar{\text{r}}$}}), it is probabilistically easy to determine whether {\pbold{G}}\textsuperscript{\pbold{$\bar{\text{p}}\bar{\text{q}}$}} = {\pbold{G}}\textsuperscript{\pbold{$\bar{\text{r}}$}}. In other words, given {\pbold{G}}\textsuperscript{\pbold{$\bar{\text{p}}$}} and {\pbold{G}}\textsuperscript{\pbold{$\bar{\text{q}}$}}, while it is hard to determine {\pbold{G}}\textsuperscript{\pbold{$\bar{\text{p}}\bar{\text{q}}$}}, it is relatively easier to\vspace{1mm}}}}\linebreak
	\colorbox{Gray}{\hspace{1mm}\parbox{1.0\textwidth}{\vspace{1mm}\textls[-50]{\mono{determine whether a third randomly chosen element {\pbold{G}}\textsuperscript{\pbold{$\bar{\text{r}}$}} is a result of {\pbold{G}}\textsuperscript{\pbold{$\bar{\text{p}}\bar{\text{q}}$}}. While the relative ease of solving D-DHP may not be obvious, the proof of it becomes apparent after taking into account the Law of Quadratic Reciprocity of Legendre symbols [9]; this however falls out of scope of this paper.\vspace{1mm}}}}}\linebreak\linebreak\linebreak
	...
	\begin{center}
		{\mono{{\pbold{y}}\textsuperscript{2} = {\pbold{x}}\textsuperscript{3} + p\,\cdot\,{\pbold{x}} + q, \\
		\vspace{2mm}
		\textls[-50]{\mono{given}} 4\,p\textsuperscript{3} + 27\,q\textsuperscript{2} $\neq$ 0
		}}
	\end{center}
	\begin{flushright}
		{\vspace{-7.5mm}\mono{(1)}}
...
\begin{multicols}{2}
	...
	\begin{minipage}{\linewidth}
		\centering
		\includegraphics[width=90.10mm]{./img/ellipticCurve.png}
	...
	\begin{minipage}{\linewidth}
		\centering
		\includegraphics[width=85mm]{./img/ECC.png}
...
\begin{flushleft}
	...
	\colorbox{Gray}{\hspace{1mm}\parbox{1.0\textwidth}{\vspace{1mm}\textls[-50]{\mono{In literature, you will often find the operation $\oplus$ equated to the generic addition $+$. This is not surprising since equations (2) and (3) resemble a + b = c and a + 0 = a for generic real numbers; 0 is the identity element of the $+$ operation. Having said that, in practice $\oplus$ operation is nothing like the addition operation $+$; the apparent similarity lies in their algebraic representations only. For instance, equations (2) and (3) also resemble a $\times$ b = c and a $\times$ 1 = a for generic real numbers. In this case, 1 is the identity element of the $\times$ operation.\vspace{1mm}}}}}\linebreak
	...
	\colorbox{Gray}{\hspace{1mm}\parbox{1.0\textwidth}{\vspace{1mm}\textls[-50]{\mono{The name of the inverse problem for ECC, i.e. discrete logarithm problem (DLP), is highly misleading and results from purely notational reasons. The term 'logarithm' originates from interpreting the result of K times repetition of {\pbold{a}} $\oplus$ {\pbold{a}} operation such that K\textsubscript{pub} = {\pbold{a}}\textsuperscript{K}.\vspace{1mm}}}}}\linebreak\linebreak\linebreak
	 \textls[-50]{\mono{It now appears that we have achieved what we set out to do: we have found a new group who elements are now {\pbold{y}} in prime field F\textsubscript{\pbold{p}} evaluated over the prime finite field and the new group operation (...(({\pbold{a}}\textsubscript{o} $\oplus$ {\pbold{a}}\textsubscript{o})\textsubscript{1} $\oplus$ {\pbold{a}}\textsubscript{o})\textsubscript{2} $\oplus$ {\pbold{a}}\textsubscript{o})\textsubscript{3} ...)\textsubscript{K} $\equiv$ {\pbold{a}}\textsubscript{o}\hspace{-1.7mm}\textsuperscript{K} is very difficult to invert. We note that the result of $\oplus$ operation on the group elements results in a value which is also a member of the group\textsuperscript{\textcolor{blue}{4}}\blfootnote{\textls[-50]{\mono{\textsuperscript{4}The group is referred to as 'closed under the operation $\oplus$'}}}; this was not the case in RSA scheme where the result {\pbold{a}}$\,\times\,${\pbold{b}} is never a member of the group.\linebreak\linebreak
	}}
	\textls[-50]{\mono{Curious observers may note that unlike RSA where we keep both {\pbold{a}} and {\pbold{b}} secret (and reveal their product {\pbold{a}}$\,\times\,${\pbold{b}}), in ECC we additionally also reveal one of the independent variables {\pbold{a}}\textsubscript{o}, i.e. {\pbold{a}}\textsubscript{o}\hspace{-1.7mm}\textsuperscript{K} and {\pbold{a}}\textsubscript{o} are both known, leaving only K to be discovered. Why is this the case? The answer lies in the fact that the solutions {\pbold{a}} and {\pbold{b}} of the product {\pbold{a}}$\,\times\,${\pbold{b}} are unique while the solutions {\pbold{a}} and K of the expression {\pbold{a}}\textsuperscript{K} are not unique unless either {\pbold{a}} or K is specified. Uniqueness of the keypairs is a very important property that cannot be violated for cryptosystems; this requires {\pbold{a}}\textsubscript{o} to be fixed and specified publicly in ECC algorithms.\linebreak\linebreak
	}}
	\textls[-50]{\mono{Curious observers may additionally note that the expression {\pbold{a}}\textsuperscript{K} looks very similar to the {\pbold{G}}\textsuperscript{K} expression in Diffie-Hellman. How similar are these two? Quite a lot actually. The group defined by {\pbold{a}}\textsuperscript{K} is in fact also a finite cyclic group with {\pbold{a}}\textsubscript{o} as its generator element! The order N, i.e. number of elements in the group, is defined by the expression {\pbold{a}}\textsubscript{o}\hspace{-1.7mm}\textsuperscript{N} = \^I = $\infty$. Does this mean we can construct an even safer Diffie-Hellman Key Sharing algorithm by using an elliptic curve to construct the cyclic field instead of the simple choice made in classical Diffie-Hellman? The answer is yes we can and such methods already exist in the form of Elliptic Curve Diffie-Hellman algorithms [8].\linebreak\linebreak
	}}
	...
	\textls[-50]{\mono{Pairing-based cryptography (PBC) is an extension of ECC which historically stems from Diffie-Hellman implementation of ECC and forms a crucial prelude to ZKC. PBC is around the point in literature when people start to lose their mind; most of this is a result of notational abuse and use of unintuitive terms such as r-torsion and divisor subgroups. The core premise of PBC is not very far from ECC and easily understood without losing our collective minds. In this paper, we will try to understand pairing-based elliptic curve cryptography using the example of a naive but intuitive cyclic group: e\textsuperscript{i\pbold{a}} in the complex space, i.e. i\textsuperscript{2} + 1 = 0.}}\linebreak\linebreak
	\colorbox{Gray}{\hspace{1mm}\parbox{1.0\textwidth}{\vspace{1mm}\textls[-50]{\mono{The vector notation e\textsuperscript{i\alpha} of this group is the usual complex number sin(\alpha) + i$\,\cdot\,$cos(\alpha), which is simply a vector in complex space inclined at an angle \alpha with the x-axis. Varying the value of \alpha in this geometric representation is equivalent to rotating the vector by some angle about the origin. Quite naturally, any resulting finite group from e\textsuperscript{i$\,\cdot\,$k\alpha} is cyclic for any generator value of {\alpha} since all of its elements can be derived by simply by varying k, i.e. rotating the generator vector incremently by \alpha.
	}}}}\linebreak\linebreak\linebreak
	\colorbox{Gray}{\hspace{1mm}\parbox{1.0\textwidth}{\vspace{1mm}\textls[-50]{\mono{This is a good time to make some additional observations about finite cyclic groups using the simple example of e\textsuperscript{i$\,\cdot\,$k\alpha}, equivalent to e\textsuperscript{k$\,\cdot\,${\pbold{z}}} where {\pbold{z}} is a complex number. The resulting group is then simply vectors in complex plane which rotate (say, counter-clockwise) around the origin in increments of {\alpha} as k varies. If {\alpha} is a factor of 2{\uppi}, then any vector starting from k = 0 will conincide with itself after precisely N = 2{\uppi}/{\alpha} turns. In other words, if we were to take any starting vector {\pbold{z}}({\alpha}\textsubscript{o}) as the identity element, then the resulting group is cyclic with an order N = 2{\uppi}/{\alpha}; all of its elements can be derived by simply rotating the generator vector e\textsuperscript{i$\,\cdot\,$\alpha\textsubscript{o}} in increments of {\alpha} up to N times. \vspace{1mm}
	}}}}\linebreak\linebreak\linebreak
	\textls[-50]{\mono{Pairing-based cryptography is entirely reliant 
	}}\linebreak\linebreak
	\textbf{\large\pbold{RECAP}}\linebreak\linebreak
	\textls[-50]{\mono{Before moving forward, let's recap the process of our rediscovery of cryptography. Cryptosystems are motivated by the need to prove access to information without revealing a part or the entirety of the said information; such systems form the building blocks of countless cryptographic methods such as encryption/decryption, permissionless access, secure key sharing, digital signatures, blockchains etc. Such information-preserving proving algorithms are called 'trapdoors' since such algorithms - similar to a trapdoor - are easy to compute in one direction but difficult to invert. RSA is one of the simplest implementations of such trapdoors and it relies on the (non-)invertibility of product operation {\pbold{a}}$\,\times\,${\pbold{b}} on a prime finite field; this securing property is called the prime factorisation problem. RSA algorithm is relatively safe to use at the moment but it is expected that it will be broken within the next decade. ECC is the next generation of cryptographic trapdoor algorithms which essentially supercharges the prime factorisation problem by several orders of magnitude. It does so by resampling the group elements from an elliptic curve evaluated over a prime finite field and defining a new operation $\oplus$ on this cyclic group which captures the supercharging effect composed of large number of atomic prime factorisations at its core. The security of ECC relies on the difficulty of discrete logarithm problem, i.e. how many $\oplus$ group operations on the generator element {\pbold{a}}\textsubscript{o} does it take to arrive at a point {\pbold{a}}\textsubscript{o}\hspace{-1.7mm}\textsuperscript{K} on the elliptic curve. The supercharging increases the security of the resulting trapdoor but fails to increase it beyond the post-quantum regime due to being limited by the atomic difficulty of prime factorisation problem.\linebreak\linebreak
	}}
	\textls[-50]{\mono{While the difficulty of ECC can be further channeled by choosing specific elliptic curves, gigabrains realised couple of decades ago that choosing a different ECC doesn't resolve the fundamental issue that plagues ECC. At the very core, solving discrete logarithm problem for (i.e. calculate K given {\pbold{a}}\textsubscript{o}\hspace{-1.7mm}\textsuperscript{K}) is bruteforceable since it reveals too much information in the form of generator element {\pbold{a}}\textsubscript{o}; this is necessary for the uniqueness of keypairs but it unwittingly reveals information that helps post-quantum bruteforce algorithms. Is this avoidable in any way? It doesn't appear so since knowing {\pbold{a}}\textsubscript{o} publicly is necessary for deriving public keys by independent parties. It appears that to truly make progress, we may have to abandon our primary use of elliptic curves. Enter zero-knowledge cryptography.
