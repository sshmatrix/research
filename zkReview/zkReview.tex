...
\begin{flushleft}
	...
	\colorbox{Gray}{\hspace{1mm}\parbox{1.0\textwidth}{\vspace{1mm}\textls[-50]{\mono{In a finite cyclic group with N elements (i.e. of "order" N), all N elements can be derived by repeatedly performing the group operation on one of the elements (called the generator). We can thus represent the K-th element in such a group with notation {\pbold{G}}\textsuperscript{K}, representing K group operations on the generator element {\pbold{G}}; note that the term {\pbold{G}}\textsuperscript{K} should not be interpreted as a literal exponent although a lot of literature uses this terminology. The use of exponential notation results from the classical Diffie-Hellman algorithm using the literal exponent of a 'primitive root modulo {\pbold{p}}' G = G\textsuperscript{K} as the group operation on prime finite fields, i.e. {\pbold{G}}\textsuperscript{K} $\equiv$ (G \bm{$\times$} G \bm{$\times$} G \bm{$\times$} G \bm{$\times$}\,{\cdot\cdot\cdot}\, \bm{$\times$} G)\textsubscript{K = 0,\,1,\,2,\,3\,\;{\cdot\cdot\cdot}\,\;(N - 1)}. The final form of {\pbold{C}}({\pbold{p}}) thus reads {\pbold{G}}\textsuperscript{K} \% {\pbold{p}}. The cyclicity of e\textsuperscript{i\pbold{z}} in complex space is another reason for the exponential notation.\vspace{1mm}}}}}\linebreak\linebreak\linebreak
	\textls[-50]{\mono{In practise, the private key for two parties is derived by evaluating the function {\pbold{C}}({\pbold{p}}) = {\pbold{G}}\textsuperscript{K} \% {\pbold{p}} from commonly known values of {\pbold{G}} and {\pbold{p}} at two values of K = {\updelta},\,{\uptheta}. The shared key then takes the form of {\pbold{G}}\textsuperscript{\updelta\uptheta} \% {\pbold{p}}, which is also the non-invertible term securing the protocol\textsuperscript{\textcolor{blue}{2}}\blfootnote{\textls[-50]{\mono{\textsuperscript{2}{Note that {\updelta} and {\uptheta} are not necessarily prime numbers}}}} instead of a simple product between two large prime numbers. Breaking the Diffie-Hellman method is thereby different from RSA since it involves inverting {\pbold{G}}\textsuperscript{\updelta\uptheta} \% {\pbold{p}} instead of {\upalpha}\bm{$\;\times\;$}{\upgamma}; this is known as the Computational Diffie-Hellman Problem (C-DHP).\linebreak\linebreak
	}}
	\colorbox{Gray}{\hspace{1mm}\parbox{1.0\textwidth}{\vspace{1mm}\textls[-50]{\mono{More specifically, C-DHP states that given the knowledge of {\pbold{G}}\textsuperscript{\updelta} and {\pbold{G}}\textsuperscript{\uptheta} (but not {\updelta} and {\uptheta}), it is difficult to compute the value of {\pbold{G}}\textsuperscript{\updelta\uptheta}. More simply, if we are revealed the values of two elements derived from secretly taking \updelta} and {\uptheta} cyclic operations, then it is difficult to compute the element derived by taking {\updelta}\bm{$\;\times\;$}{\uptheta} cyclic operations. One variant of Diffie-Hellman problem which is easy to solve in this case is the Decisional Diffei-Hellman Problem (D-DHP). D-DHP states that given knowledge of three elements {\pbold{G}}\textsuperscript{\updelta}, {\pbold{G}}\textsuperscript{\uptheta} and {\pbold{G}}\textsuperscript{\upsigma} (but not {\updelta}, {\uptheta} or {\upsigma}), it is probabilistically easy to determine whether {\pbold{G}}\textsuperscript{\updelta\uptheta} = {\pbold{G}}\textsuperscript{\upsigma}. In other words,\vspace{1mm}}}}\linebreak
	\colorbox{Gray}{\hspace{1mm}\parbox{1.0\textwidth}{\vspace{1mm}\textls[-50]{\mono{given {\pbold{G}}\textsuperscript{\updelta} and {\pbold{G}}\textsuperscript{\uptheta}, while it is hard to determine {\pbold{G}}\textsuperscript{\updelta\uptheta}, it is relatively easier to determine whether a third randomly chosen element {\pbold{G}}\textsuperscript{\upsigma} is a result of {\pbold{G}}\textsuperscript{\updelta\uptheta}. While the relative ease of solving D-DHP may not be obvious, the proof of it becomes apparent after taking into account the Law of Quadratic Reciprocity of Legendre symbols [9]; this however falls out of scope of this paper.\vspace{1mm}}}}}\linebreak\linebreak\linebreak
...
\begin{multicols}{2}
	...
	\begin{minipage}{\linewidth}
		\centering
		\includegraphics[width=80.6mm]{./img/ellipticCurve.png}
	...
	\begin{minipage}{\linewidth}
		\centering
		\includegraphics[width=78mm]{./img/ECC.png}
