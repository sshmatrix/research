...
\begin{flushleft}
	...
	\textbf{\Large\pbold{INITIATION BY JACOBI}}\linebreak\linebreak
	...
	\end{flushright}
	\vspace{2mm}
	which is eerily innocent-looking and quite similar to (3) except that the integrand is inverted. This particular form of the elliptic integral of the first kind circle is where Jacobi made the first meaningful step toward parameterising generic elliptic curves. While generalising the integral F(x) to F(e,\,x) for non-zero e, Jacobi concluded that he didn't in fact need to solve the integral but instead simply attempt to gauge its properties (e.g. periodicity) by analysing the integrant. Jacobi's playtime revealed that the symmetric appearance of the terms e\textsuperscript{2} and x\textsuperscript{2} in the general elliptic integral equates to a bi-periodic function with complex periods! In simpler words, equation (7) is double-periodic with both periods ($\hat{\upomega}$\textsubscript{1},\,$\hat{\upomega}$\textsubscript{2}\;{$\in$}\;{$\mathbb{C}$}) being complex numbers, such that C($\bar{\text{x}}$) = C($\bar{\text{x}}$ + n\textsubscript{1}\,\cdot\,$\hat{\upomega}$\textsubscript{1}) = C($\bar{\text{x}}$ + n\textsubscript{2}\,\cdot\,$\hat{\upomega}$\textsubscript{2}), where $\hat{\upomega}$\textsubscript{1} and $\hat{\upomega}$\textsubscript{2} must necessarily be linearly-independent in {$\mathbb{R}$}, aka they are not mutually related by  $\hat{\upomega}$\textsubscript{1}\,{\robot{^^^^2260}}\;{$\mathbb{R}$}\,\cdot\,$\hat{\upomega}$\textsubscript{2}. Jacobi had arrived at this result after he fucked around with C($\bar{\text{x}}$) and assumed it to be a sine-inverse-like function{\textsuperscript{\textcolor{blue}{2}}\blfootnote{\textls[-50]{\mono{\textsuperscript{2}{due to the said integral being literally sin\textsuperscript{-1}($\bar{\text{x}}$) for e = 0}}}}} (denoted sin\textsuperscript{-1}\hspace{-3.0mm}\textsubscript{L}\hspace{1.3mm}$\bar{\text{x}}$), and found out that it did in fact have properties similar to a sinusoid such as periodicity. This however wasn't the most important assertion; Jacobi further found that no single-variable function has more than two independent periods, and all except elliptic integrals of the first and second (and third) kind have at most one period only. This was arguably the first solid proof that elliptic integrals were a special class of functions.\linebreak\linebreak
	}}
	\colorbox{Gray}{\hspace{1mm}\parbox{1.0\textwidth}{\vspace{1mm}\textls[-50]{\mono{Note that in order to arrive at (4), we needed to make the seemingly absurd parameterisation {\uptheta} = 1 - e\textsuperscript{2}x\textsuperscript{2}. This parameterisation, while unusual, has merits to it; the trick to understanding it lies in the parameterisation (x, y) {\rightarrow} ({\upalpha}\,sin{\,\uptheta}, cos{\,\uptheta}). In particular, the term 1 - e\textsuperscript{2}x\textsuperscript{2} is similar to the series form of cos{\,\uptheta} = 1 - $\frac{\small\text{1}}{\small\text{2}}$x\textsuperscript{2} + $\frac{\small\text{1}}{\small\text{12}}$x\textsuperscript{4} + ... truncated at the second term and with an additional absurd equivalence e {\asserts} 1/$\sqrt{\text{2}}$. While this equivalence is indeed absurd, it didn't stop Jacobi from realising that he could define another trigonometric function similar to cos{\,\uptheta}, denoted cos\textsubscript{e}{\uptheta}, such that when truncated at the second term, it yields 1 - e\textsuperscript{2}x\textsuperscript{2}. We can brainstorm this right now; it seems like we can get a term like 1 - e\textsuperscript{2}x\textsuperscript{2} by multiplying 1 {\robot{^^^^00b1}}\;\,e\textsuperscript{2} and 1 {\robot{^^^^00b1}}\;\,x\textsuperscript{2}, and truncating it at the second term. This is not sufficient though; we need to arrive at the '-' sign and somehow take care of the $\frac{\small\text{1}}{\small\text{2}}$ factors in the original cosine series. It is somewhat easy to ensure arriving at '-' sign by introducing the complex unit {\mbold{i}}\textsuperscript{2} = -1, while the factors of $\frac{\small\text{1}}{\small\text{2}}$ can be - \vspace{1mm}}}}}\linebreak\linebreak\linebreak
...
\begin{multicols}{2}
	...
	\begin{minipage}{\linewidth}
		...
		\captionof{figure}{\textls[-50]{\mono{Families of Elliptic curve}}}
		\label{fig:ec}
	\end{minipage}
	\noindent
	\begin{minipage}{\linewidth}
		...
		\captionof{figure}{\textls[-50]{\mono{Elliptic curve operation}}}
		\label{fig:ecc}
	\end{minipage}
\end{multicols}
\pagebreak
\begin{flushleft}
	\textls[-50]{\mono{This is not the only way to arrive at elliptic curves
	}}
	\linebreak\linebreak\linebreak
